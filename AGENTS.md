...
- **Radius belongs to the theme preset — respect it.** The dashboard-01 preset ships rounded primitives (`rounded-4xl` buttons/badges/dialogs, `rounded-2xl` cards, `--radius: 0.625rem`); that rounded look is intentional, not a bug. Don't hardcode `rounded-md`, `rounded-lg`, or similar Tailwind radius classes in app/feature markup that fight the preset — leave radius to the primitives. The one deliberate exception: avatars/profile initials use `rounded-full` — that's a shape decision, not a corner-radius override, and it's fine for it to be the one round element style.
